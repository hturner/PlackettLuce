...
pladmm <- function(rankings, # rankings object as used in PlackettLuce
    ...
    conv <- FALSE
    beta_iter <- inits$exp_beta_init
    pi_iter <- softmax(X %*% beta_iter)
    u_iter <- inits$u_init
    ...
    for (iter in seq_len(n_iter)){
        ...
        if (!conv){
            pi_prev <- pi_iter
            beta_prev <- beta_iter
            tilde_pi_prev <- softmax(X %*% beta_iter)
            ...
            tilde_pi_iter <- softmax(X %*% beta_iter)
