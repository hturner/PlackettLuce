...
#' @param log a logical indicating whether to return worth parameters on the
#' log scale with the item specified by \code{ref} set to zero.
...
coef.PlackettLuce <- function(object, ref = 1, log = TRUE,
  ...
  if (!log) {
      ...
      ref <- attr(item, "ref")
      nitem <- length(item)
      coefs <- c(item, object$coefficients[-seq_len(nitem)])
  }
  ncoefs <- length(coefs)
