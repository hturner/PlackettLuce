...
#' @export
testb <- function(rankings, ref = NULL,
    ...
    key_quantities <- function(par) {
        ...
        res <- expectation2b("all", alpha, c(1, delta), R, S, N, D, P)
    ...
    res[c("expA", "expB")] <-
        expectation2b("all", alpha, delta,  R, S, N, D, P)[c("expA", "expB")]
    oneUpdate <- function(res){
        ...
        if (D > 1) res$delta[-1] <- B[-1]/
                expectation2b("delta", res$alpha, res$delta, R, S, N, D, P)$expB
        res[c("expA", "expB")] <-
            expectation2b("all", res$alpha, res$delta, R, S, N, D, P)[c("expA", "expB")]
        res
    }
