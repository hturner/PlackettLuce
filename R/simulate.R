...
#' @param max_choices a positive number (default is
#'     \code{sum(choose(30, 1:10))}). See Details.
...
#' @export
simulate.PlackettLuce <- function(object, nsim = 1, seed = NULL, multinomial = FALSE, max_choices = 616665, ...) {
    ...
    if (object$maxTied == 1 & !multinomial) {
        ...
        ## a preventive stop if more than max_choices need to be made per ranking
        n_choices <- sum(choose(N, seq_len(max(len))))
print(n_choices)
        if (n_choices > max_choices) {
            stop(paste("rankings are generated by", n_choices, " choices, but cannot proceed with more than", max_choices))
        }
