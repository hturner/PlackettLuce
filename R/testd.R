...
#' @export
testd <- function(rankings, ref = NULL,
    ...
    key_quantities <- function(par) {
        ...
        res <- expectation2d("all", alpha, c(1, delta), R, S, N, D, P)
    ...
    res[c("expA", "expB")] <-
        expectation2d("all", alpha, delta,  R, S, N, D, P)[c("expA", "expB")]
    oneUpdate <- function(res){
        ...
        if (D > 1) res$delta[-1] <- B[-1]/
                expectation2d("delta", res$alpha, res$delta, R, S, N, D, P)$expB
        res[c("expA", "expB")] <-
            expectation2d("all", res$alpha, res$delta, R, S, N, D, P)[c("expA", "expB")]
        res
    }
