...
Then under Luce's axiom, the probability of selecting some item $i$
from $S_j$ is given by

$$P(i | S_j) = \frac{\alpha_{i}}{\sum_{c \in S_j} \alpha_c}$$

where $\alpha_i$ represents the **worth** of item $i$. Viewing a ranking of $K$
...
some preparation to collate the rankings, which is detailed in the Appendix
Appendix. The same code is provided in the example of in the help file of
`beans`
```{r beans-preparation, eval = FALSE}
...
```{r beans-1, echo = FALSE, results = FALSE}
data(beans)
head(beans[c("best", "worst")], 2)
```
```{r beans-2, echo = FALSE, results = FALSE}
beans <- within(beans, {
...
```
```{r beans-3, echo = FALSE, results = FALSE}
varieties <- as.matrix(beans[c("variety_a", "variety_b", "variety_c")])
head(varieties, 2)
```
```{r beans-4, echo = FALSE, results = FALSE}
n <- nrow(beans)
beans <- within(beans, {
...
head(paired[[id]])
```
```{r beans-8, echo = FALSE, results = FALSE}
...
`grouped_rankings` object as follows:
```{r}
G <- grouped_rankings(R, rep(seq_len(nrow(beans)), 4))
format(head(G, 2), width = 50)
```
For each record in the original data, we have three covariates: `season` for the
season-year the beans were planted, `year` for the year of planting, and
`rx5day` for the maximum 5-day rainfall.
```{r}
...
These covariates and the are used to fit a Plackett-Luce tree for the
`grouped_rankings` object `G`
```{r beans-tree}
tree <- pltree(G ~ season + year + rx5day, data = beans,
               minsize = 5, maxit = 500)
tree
```
The resulting tree has two nodes defined by the seasons.
```{r}
plot(tree)
```
...
First we consider the best and worst rankings. These give the variety the farmer
though was best or worst, coded as A, B or C for the first, second or third
variety assigned to the farmer respectively.
```{r}
```{r beans-1, eval = FALSE}
```
