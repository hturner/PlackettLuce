...
different packages. See Appendix \\@ref(app:timings) for details and code.", escape = FALSE) %>%
    kable_styling() %>%
...
\@ref(eq:PL). In the next section we discuss why it is more appropriate to put
the worth parameters on the log scale for inference.
...
preferences presented in @Davidson1970. The tie parameter is still shown on the
original scale.
