...
the extent of this support is not clear. The timings in Table \@ref(tab:nascar) for fitting the Plackett-Luce model on the NASCAR data from @Hunter2004 illustrate that **PlackettLuce** is more efficient than
...
The model-fitteding function in **PlackettLuce** requires data in the form of 
...
the combined orderings to rankings, passing the ranking frequencies to store 
...
The print method for aggregated rankings displays the unique 
rankings in a readable form. The underlying data structure stores the 
rankings in the form required for modelling, which can be seen by converting 
the rankings object to a matrix:

```{r}
head(as.matrix(R), 2)
```

Columns 1-6 represent the pudding brands. In each row, `0` represents an
...
We can fit a Plackett-Luce model to these rankings using the `PlackettLuce` 
function from **PlackettLuce**. By default, the frequencies stored in the 
aggregated rankings object are used as the model weights, as shown explicitly 
in the call below. Setting `npseudo = 0` means that standard
...
```{r}
mod <- PlackettLuce(R, weights = freq(R), npseudo = 0, maxit = 7)
coef(mod, log = FALSE)
```
