...
- name: David Firth 
  affiliation: Department of Statistics, University of Warwick, UK; and Alan Turing Institute, London, UK
- name: Ioannis Kosmidis
  affiliation: Department of Statistical Science, UCL, UK; and Alan Turing Institute, London, UK
...
abstract: |
  ...
  (rankings of only some items). By default, the implementation adds a set of
...
represent the presence or absence of each item in each choice and a factor 
...
[//]: # (Is that last comment really just related to the stopping rule for PLMIX, though?  DF)
...
items are unranked, but implicitly ranked lower than the top $n$. Sub-rankings
...
the facility to model heterogeneous populations of subjects that have different
...
[//]: # (Do we need/want to give the full detail there?  It gets a bit complicated.  I guess we'll need that for version to submit for publication, though.  DF)
...
which `PlackettLuce` create internally if necessary:
...
None of the Z tests for the item parameters is significant, which is consistent
with test for equal preferences presented in @Davidson1970. Note the tie 
parameter is also shown on the log scale here, but since it is an integral part 
of the model it is not a parameter of interest for inference, so the scale is 
not relevant.
...
changed via a `ref` argument. 
The plot method for the returned object visualizes
...
The `adjacency` function from **PlacketLuce** can be used to convert these to
an adjacency matrix where element $(i, j)$ is the number of times item $i$ is 
...
```
For the worth parameters (on the log scale) to have finite maximum likelihood estimates (MLEs) and 
...
[//]: # (I have corrected what I think were errors in the above two paragraphs, in the statements about finiteness of estimates.  This is most straightforwardly stated on the log scale, I think.  DF)
...
the network is returned. In this case a model could be estimated by excluding item
D:
```{r}
R2 <- R[, -4]
R2
...
parameters changes quite considerably:
```{r}
mod2 <- PlackettLuce(R)
coef(mod2)
```
...
shrinkage effect, shrinking the items' worth parameters towards $1/N$, where $N$ is the
number of items.
...
because the small shrinkage effect that the pseudo-data provides 
also reduces both the bias and the variance of the estimates.
...
This is an extension of Bradley-Terry trees, implemented in the R package **psychotree** and described in more detail by @Strobl2011.
...
The data are provided as the data set `beans` in Plackett-Luce. The data require 
some preparation to collate the rankings. First we consider the best and 
worst rankings. These give the variety the farmer thought was best or worst, 
...
the original data set, so we create a \code{"grouped_rankings"} object that 
identifies the record number for each of the four rankings from each farmer (
one ranking of order three plus three pairwise rankings with the local variety):
```{r}
...
and \code{maxTN} the maximum temperature at night during the vegetative cycle. 
