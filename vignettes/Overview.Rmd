...
(ref:features) Features of example data sets from PrefLib [@Mattei2013]
...
(ref:timings) Timings for fitting the Plackett-Luce model to data sets summarised in 
Table \@ref(tab:data-features) using different packages. See Appendix \@ref(app:timings) for details and code.

```{r timings-kable, echo = FALSE}
res <- data.frame(netflix = round(unlist(netflix_timings), 3),
                  tshirt = round(unlist(tshirt_timings), 3),
...
(ref:nascar) Timings for fitting the Plackett-Luce model to the NASCAR data from @Hunter2004. All rankings are unique.
...
however there are some advantages to creating a formal `"rankings"` object,
...
method for `"PlackettLuce"` objects sets the first item as the reference:
...
For the worth parameters (on the log scale) to have finite 
...
based on judge covariates. A Plackett-Luce tree is constructed via the following
steps:
...
we wish to group by each record in the original data set, so we create a
`"grouped_rankings"` object that 
...
partial-rankings and accounting for spatio-temporal heterogeneity in worth
parameters.
...
variety (a strict ranking is assumed, so the sum of each row must be 6)
```{r beans-2}
beans <- within(beans, {
