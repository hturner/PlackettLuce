...
abstract: |
  ...
  a regularization effect, shirnking the estimated parameters towards equal item worth. In addition to
...
the extend of this support is not clear. The timings in Table \@ref(tab:nascar) for fitting the Plackett-Luce model on the NASCAR data from @Hunter2004 illustate that **PlackettLuce** is more efficient than
...
coef(mod, log = FALSE)
```
Note here that we have specified `log = FALSE` in order to report the estimates in the parameterization of Equation
...
method for `"PlackettLuce"` objects sets the alphabetically first item as the reference:
...
The `adjacency` function from **PlackettLuce** can be used to convert these to an
...
because the small shrinkage effect that the pseudo-data delivers
has also a beneficial impact in terms of reducing the bias and the variance of the estimators.
...
identifies the record number for each of the four rankings from each farmer (one ranking of order three plus three pairwise rankings with the local variety):
```{r}
