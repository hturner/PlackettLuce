...
library(BradleyTerry2) # flatlizards
...
test_that("PLADMM itempar works for standard PL [salad pairs]", {
    ...
    expect_equal(sqrt(diag(attr(itempar(res0_PLADMM, ref = 1, log = TRUE),
                                "vcov"))),
...
test_that("PLADMM itempar works for PL with covariates [salad pairs]", {
    ...
    V2 <- D %*% V %*% t(D)
    expect_equal(unname(sqrt(diag(attr(itempar(res_PLADMM, ref = 1, log = TRUE),
                                "vcov")))),
                 unname(sqrt(diag(V2))),
                 coef_tol)
